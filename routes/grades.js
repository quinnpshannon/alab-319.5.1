...
router.get("/:id", async (req, res) => {
  ...
  console.log(grade);
  if (!grade) res.send("Not found").status(404);
...
router.patch("/:id/remove", async (req, res) => {
  const grade = await Grades.findById(req.params.id);
  const removed = grade.scores.pull(req.body.assignment_val);
  await grade.save();
  if (!grade) res.send("Not found").status(404);
  else res.send(removed).status(200);
});
